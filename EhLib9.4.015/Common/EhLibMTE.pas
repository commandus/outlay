...
{                    (Build 9.4.09)                     }
{                                                       }
{    Copyright (c) 2003-2019 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        Result := False;
        CheckValueProc(AGrid.Columns[FCurInListColIndex], Result);
        if Result then
        begin
          AGrid.SelectedIndex := AGrid.Columns[FCurInListColIndex].Index;
          Break;
        end;
      end else
      begin
  ...
  begin
    TDBGridEh(Sender).DataSource.DataSet.Filter :=
      GetExpressionAsFilterString(TDBGridEh(Sender),
        ...
        SupportsLocalLike, NullComparisonSyntax(TDBGridEh(Sender), DataSet), IsInSupported);
